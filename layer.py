...
class Layer(object):
    ...
    def randomizeWeightsandBiases(self):
        ...
        if self.inputNeuronCount > 0:
            ...
            self.weights = th.shared(value=(self.sharedRandomStream.normal(
                [self.inputNeuronCount, self.neuronCount]).eval().copy()), name='w')
        ...
        if self.sharedBias:
            ...
            randBiases = self.sharedRandomStream.normal(
                [self.neuronCount]).eval().copy()
            self.biases = th.shared(value=randBiases, name='b')
    ...
    def calculateActivations(self):
        if self.inputNeuronCount > 0:
            ...
            """
            self.activations = T.nnet.sigmoid(
                T.dot(self.inputLayer.activations, self.weights) + self.biases)
    ...
    def sgd(self):
        ...
        self.updates = [	(self.weights, self.weights - float(self.learningRate) * self.gw),
                         (self.biases, self.biases -
                          float(self.learningRate) * self.gb)
                         ]
...
class LayerTest(unittest.TestCase):
    def test_init(self):
        ...
        self.assertEqual(np.shape(l1.weights),
                         (l1.neuronCount, l1.inputNeuronCount))
